...
import { getPublishedNews, getUpcomingEvents, listGalleryAlbums } from '@/lib/data/cms';

export default async function HomePage() {
  const [newsArticles, eventsList, galleryAlbums] = await Promise.all([
    getPublishedNews(2),
    getUpcomingEvents(3),
    listGalleryAlbums({ publishedOnly: true, limit: 3 }),
  ]);
  return (
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="space-y-4">
              {newsArticles.map((news) => (
                <div key={news.slug} className="p-4 border border-[var(--border)] rounded bg-[var(--soft-bg)] grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                  <div className="relative h-32 w-full rounded overflow-hidden sm:col-span-1">
                    {news.coverImageUrl ? (
                      <Image src={news.coverImageUrl} alt={news.title} fill className="object-cover" unoptimized />
                    ) : (
                      <div className="h-full flex items-center justify-center">
                        <i className="bi bi-newspaper text-2xl text-[var(--muted-text)]"></i>
                      </div>
                    )}
                  </div>
                  <div className="sm:col-span-2 space-y-2">
                    <span className="text-[10px] font-bold bg-slate-200 text-slate-800 px-2 py-0.5 rounded capitalize">{news.category}</span>
                    ...
                    <p className="text-xs text-[var(--muted-text)] line-clamp-2">{news.excerpt}</p>
                    <div className="text-[11px] text-slate-400">
                      {news.publishedAt
                        ? new Date(news.publishedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
                        : 'Draft'}
                    </div>
                  </div>
                </div>
              ...
              {newsArticles.length === 0 && (
                <p className="text-xs text-[var(--muted-text)]">No news articles published yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="space-y-3">
              ...
              {eventsList.length === 0 && (
                <p className="text-xs text-[var(--muted-text)]">No upcoming events scheduled.</p>
              )}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {galleryAlbums.map((album) => (
              <Link key={album.slug} href={`/gallery/${album.slug}`} className="group bg-white border border-[var(--border)] rounded overflow-hidden">
                <div className="relative h-48 w-full">
                  {album.coverImageUrl ? (
                    <Image src={album.coverImageUrl} alt={album.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
                  ) : (
                    <div className="h-full flex items-center justify-center">
            ...
            {galleryAlbums.length === 0 && (
              <p className="text-xs text-[var(--muted-text)]">Gallery coming soon.</p>
            )}
          </div>
        </div>
      </section>
